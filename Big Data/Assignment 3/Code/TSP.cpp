	...
	while (temperature > 1000 && gen <= gen_thres) { 
		...
		for (int i = 0; i < POP_SIZE; i++) { 
			...
			while (true) { 
				...
				cout<<"\nCurrent Population : "<<p1.gnome<<" and fitness is : "<<p1.fitness<<endl<<"Mutated Gene : "<<new_gnome.gnome<<" and fitness is : "<<new_gnome.fitness;
				if (new_gnome.fitness <= population[i].fitness) { 
					...
					float prob = pow(2.7, 
									-1 * ((float)(new_gnome.fitness 
										...
										/ temperature)); 
                    cout<<"\nProbability of this new - gene is : "<<prob;
					if (prob > 0.5) { 
						new_population.push_back(new_gnome); 
						break; 
					} 
